...
export default function Home() {
  ...
  return (
    <main className={`min-h-screen transition-colors duration-75 relative overflow-x-hidden font-sans ${isLightMode ? 'bg-[#f4f6f8] text-[#0a0a0c]' : 'bg-[#050505] text-white selection:bg-[#8338EC] selection:text-white'}`}>
      
      {/* --- SABİT ARKA PLAN (HOLLOW PURPLE) --- */}
      <div className="fixed inset-0 z-0 pointer-events-none overflow-hidden">
        <div className={`absolute top-[-10%] left-[-10%] w-[50vw] h-[50vw] bg-[#2283C4] rounded-full filter blur-[150px] animate-[pulse_6s_ease-in-out_infinite] transition-opacity duration-75 ${isLightMode ? 'opacity-[0.04]' : 'opacity-10'}`}></div>
        <div className={`absolute bottom-[-10%] right-[-10%] w-[50vw] h-[50vw] bg-[#D52F2F] rounded-full filter blur-[150px] animate-[pulse_6s_ease-in-out_infinite_reverse] transition-opacity duration-75 ${isLightMode ? 'opacity-[0.04]' : 'opacity-10'}`}></div>
        <div className={`absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[60vw] h-[60vw] bg-[#8338EC] rounded-full filter blur-[200px] transition-opacity duration-75 ${isLightMode ? 'opacity-[0.02]' : 'opacity-[0.07]'}`}></div>
      </div>

      {/* --- YUKARIDAN İNEN SABİT MENÜ (STICKY NAVBAR) --- */}
      <header className={`fixed top-0 left-0 w-full z-40 transition-all duration-75 px-6 py-4 flex justify-between items-center ${isScrolled ? (isLightMode ? 'bg-white/80 backdrop-blur-md shadow-sm border-b border-gray-200 translate-y-0' : 'bg-[#0a0a0c]/90 backdrop-blur-md border-b border-white/5 translate-y-0') : 'opacity-0 pointer-events-none -translate-y-full'}`}>
      ...
      <button 
        ...
        className={`fixed top-5 right-6 z-50 w-16 h-8 rounded-full p-1 flex items-center transition-all duration-75 shadow-lg ${isLightMode ? 'bg-[#e2e8f0] border border-gray-300 hover:shadow-xl' : 'bg-[#121215] border border-white/10 hover:shadow-[0_0_15px_rgba(131,56,236,0.3)]'}`}
        aria-label="Toggle Theme"
      >
        <div className={`w-6 h-6 rounded-full flex items-center justify-center transition-all duration-75 ${isLightMode ? 'bg-white translate-x-0 shadow-sm' : 'bg-gradient-to-tr from-[#8338EC] to-[#2283C4] translate-x-8'}`}>
          {isLightMode ? (
      ...
      <section className="relative z-10 min-h-screen flex flex-col items-center justify-center p-4">
        <div className="w-full max-w-4xl flex flex-col items-center justify-center gap-10 mt-[-5vh]">
          ...
          <p className={`text-lg md:text-xl max-w-xl text-center mx-auto font-light mb-4 transition-colors duration-75 ${isLightMode ? 'text-gray-600' : 'text-gray-400'}`}>
            Aklındaki karmaşık fikirleri, saniyeler içinde kusursuz bir Fandom veritabanına dönüştür.
          </p>

          <div className={`w-full max-w-2xl rounded-2xl relative overflow-hidden transition-all duration-75 border ${isLightMode ? 'bg-white/80 border-gray-200 shadow-xl focus-within:border-[#8338EC]/50 focus-within:shadow-[0_0_30px_rgba(131,56,236,0.15)]' : 'bg-[#0a0a0c]/80 backdrop-blur-md border-white/5 shadow-2xl focus-within:border-[#8338EC]/40 focus-within:shadow-[0_0_40px_rgba(131,56,236,0.1)]'}`}>
            <div className="absolute top-0 left-0 w-full h-1 bg-gradient-to-r from-[#2283C4] via-[#8338EC] to-[#D52F2F] opacity-80"></div>
            
            <textarea
              rows={5}
              className={`w-full bg-transparent text-lg p-6 focus:outline-none resize-none leading-relaxed transition-colors duration-75 ${isLightMode ? 'text-gray-900 placeholder-gray-400' : 'text-gray-200 placeholder-gray-600'}`}
            ...
            <div className="flex justify-between items-center px-6 pb-5 pt-2">
              <span className={`text-xs font-mono tracking-wider transition-colors duration-75 ${isLightMode ? 'text-gray-500' : 'text-gray-600'}`}>
        ...
        <div className={`absolute bottom-10 animate-bounce transition-colors duration-75 ${isLightMode ? 'text-gray-400' : 'text-gray-600'}`}>
      ...
      <section className="relative z-10 w-full max-w-6xl mx-auto py-32 px-6">
        <div className="text-center mb-20">
          ...
          <p className={`text-lg transition-colors duration-75 ${isLightMode ? 'text-gray-500' : 'text-gray-400'}`}>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          ...
          <div className={`backdrop-blur-sm border p-8 rounded-2xl transition-all duration-75 ${isLightMode ? 'bg-white border-gray-200 hover:shadow-xl hover:border-[#D52F2F]/30' : 'bg-[#0a0a0c]/60 border-white/5 hover:border-[#D52F2F]/30'}`}>
          ...
          <div className={`backdrop-blur-sm border p-8 rounded-2xl transition-all duration-75 ${isLightMode ? 'bg-white border-gray-200 hover:shadow-xl hover:border-[#8338EC]/30' : 'bg-[#0a0a0c]/60 border-white/5 hover:border-[#8338EC]/30'}`}>
          ...
          <div className={`backdrop-blur-sm border p-8 rounded-2xl transition-all duration-75 ${isLightMode ? 'bg-white border-gray-200 hover:shadow-xl hover:border-[#2283C4]/30' : 'bg-[#0a0a0c]/60 border-white/5 hover:border-[#2283C4]/30'}`}>
      ...
      {/* --- 3. BÖLÜM: FOOTER --- */}
      <footer className={`relative z-10 w-full border-t pt-16 pb-8 px-6 mt-12 transition-colors duration-75 ${isLightMode ? 'bg-white border-gray-200' : 'bg-[#070709] border-white/10'}`}>
        ...
        <div className={`max-w-6xl mx-auto border-t pt-8 text-center text-xs transition-colors duration-75 ${isLightMode ? 'border-gray-200 text-gray-400' : 'border-white/5 text-gray-500'}`}>
